...
bool carrier_rw::are_connections_ready(std::map<std::string, std::set<std::string>> carriers, uint32_t timeout, bool encrypted, std::map<std::string, std::set<std::string>> &missing_connections){
    bool result = false;
    FUN_TRACE;
    try{
        connection_id_map::vtr connection_ids = connection_id_map::get(carriers);
                    ...
                    if (op->connect()){
                        foreach(std::string id, c->ids){
                            ...
                            try{
                                ...
                                missing_connections[id].insert(op->addr);
                            }
                        }
                    }
                }
...
bool carrier_rw::send_checksum_info(std::set<std::string> carriers, const std::string& connection_id, const std::string& file_name, const std::string& data, uint32_t timeout, bool encrypted){
    bool result = false;
    carrier_op::vtr        carrier_ops;
    foreach(std::string _c, carriers){
        carrier_ops.push_back(carrier_op::ptr(new carrier_op(_c, timeout, encrypted, false)));
    }
    foreach(carrier_op::ptr op, carrier_ops){
        try{
            macho::windows::auto_lock lock(op->cs);
            if (op->connect()){
                if (result = op->client->send_checksum_info("", connection_id, file_name, data))
                    break;
            }
        }
        catch (saasame::transport::invalid_operation& ex){
...
std::string carrier_rw::receive_checksum_result(std::set<std::string> carriers, const std::string& connection_id, const std::string& file_name, uint32_t timeout, bool encrypted){
    std::string result;
    carrier_op::vtr        carrier_ops;
    foreach(std::string _c, carriers){
        carrier_ops.push_back(carrier_op::ptr(new carrier_op(_c, timeout, encrypted, false)));
    ...
    foreach(carrier_op::ptr op, carrier_ops){
        try{
            macho::windows::auto_lock lock(op->cs);
            if (op->connect()){
                op->client->receive_checksum_result(result, "", connection_id, file_name);
                if (!result.empty())
                    break;
            }
        }
