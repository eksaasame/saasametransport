...
bool _is_drive_letter(std::wstring volume_path){
    /*
...
disk_universal_identify::disk_universal_identify(macho::windows::storage::disk& d) : _mbr_signature(0), _unique_id_format(unique_id_format_type::Vendor_Specific){
    ...
    if ((unique_id_format_type)d.unique_id_format() == unique_id_format_type::FCPH_Name){
...
disk_universal_identify::disk_universal_identify(const std::string& uri) : _mbr_signature(0), _unique_id_format(unique_id_format_type::Vendor_Specific) {
    mValue value;
    read(uri, value);
...
bool disk_universal_identify::operator == (const disk_universal_identify& disk_uri){
    ...
    if (_unique_id_format == unique_id_format_type::FCPH_Name && _unique_id_format == disk_uri._unique_id_format && !_unique_id.empty() && !disk_uri._unique_id.empty())
        ...
        else if (_friendly_name.length() > 5 && 0 == strncmp(_friendly_name.c_str(), "/dev/", 5) )
...
void common_service_handler::get_host_detail(physical_machine_info& _return, const std::string& session_id, const machine_detail_filter::type filter){
    ...
    try{
        ...
        if (reg2.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH))){
            ...
            std::string computer_sid = macho::stringutils::convert_unicode_to_utf8(macho::windows::environment::get_computer_sid());
            if (reg2[_T("Machine")].exists()){
                bool is_validated = false;
                if (!computer_sid.empty() && reg2[_T("DigitalId")].exists()){
                    macho::bytes digital_id = reg2[_T("DigitalId")];
                    if (digital_id.length()){
                        macho::bytes u1 = macho::windows::protected_data::unprotect(digital_id, true);
                        if (u1.length()){
                            std::string checksum = std::string(reinterpret_cast<char const*>(u1.ptr()), u1.length());
                            macho::bytes check((LPBYTE)checksum.c_str(), checksum.length());
                            std::string new_checksum = macho::sha1::hmac_sha1(computer_sid, reg2[_T("Machine")].string());
                            macho::bytes newcheck((LPBYTE)new_checksum.c_str(), new_checksum.length());
                            is_validated = checksum.length() == new_checksum.length() && 0 == memcmp(checksum.c_str(), new_checksum.c_str(), checksum.length());
                            if (!is_validated){
                                LOG(LOG_LEVEL_WARNING, L"Machine ID  : %s", reg2[_T("Machine")].wstring().c_str());
                                LOG(LOG_LEVEL_WARNING, L"Checksum    : %s", check.get().c_str());
                                LOG(LOG_LEVEL_WARNING, L"Computer SID: %s", macho::stringutils::convert_utf8_to_unicode(computer_sid).c_str());
                                LOG(LOG_LEVEL_WARNING, L"New Checksum    : %s", newcheck.get().c_str());
                            }
                        }
                    }
                }
                //if (!is_validated){
                //	reg2[_T("Machine")].delete_value();
                //	reg2[_T("DigitalId")].delete_value();
                //	reg2.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH));
                //}
            }
            ...
            if (!reg2[_T("Machine")].exists()){
                ...
                foreach(macho::windows::storage::disk::ptr &d, disks){
                    std::wstring serial_number = macho::stringutils::erase_trailing_whitespaces(macho::stringutils::remove_begining_whitespaces(d->serial_number()));
                    if (d->is_system() && serial_number.length() > 4 && serial_number != L"2020202020202020202020202020202020202020"){
                        machine_id = d->manufacturer() + d->serial_number();
                        LOG(LOG_LEVEL_DEBUG, L"Machine ID key (from disk serial number): %s", machine_id.c_str());
                        break;
                    }
                }
                if (!machine_id.length()){
                    ...
                    foreach(network_info& net, nets){
                        if (!net.mac_address.empty()){
                            machine_id = macho::stringutils::convert_utf8_to_unicode(net.mac_address);
                            LOG(LOG_LEVEL_DEBUG, L"Machine ID key (from MAC address): %s", machine_id.c_str());
                            break;
                        }
                    }
                }
                ...
                if (!computer_sid.empty()){
                    std::string checksum = macho::sha1::hmac_sha1(computer_sid, reg2[_T("Machine")].string());
                    macho::bytes digital_id;
                    digital_id.set((LPBYTE)checksum.c_str(), checksum.length());
                    macho::bytes u1 = macho::windows::protected_data::protect(digital_id, true);
                    reg2[_T("DigitalId")] = u1;
                }
            }
...
service_info common_service_handler::get_service(int port){
    ...
    if (boost::filesystem::exists(p / "server.key") && boost::filesystem::exists(p / "server.crt")){
        try
        {
            boost::shared_ptr<apache::thrift::transport::TSSLSocketFactory> factory;
            factory = boost::shared_ptr<apache::thrift::transport::TSSLSocketFactory>(new apache::thrift::transport::TSSLSocketFactory());
            ...
            std::shared_ptr<apache::thrift::transport::TSSLSocket> ssl_socket = std::shared_ptr<apache::thrift::transport::TSSLSocket>(factory->createSocket("localhost", port));
            ssl_socket->setConnTimeout(1 * 1000);
            transport = std::shared_ptr<apache::thrift::transport::TTransport>(new apache::thrift::transport::TBufferedTransport(ssl_socket));
        }
        catch (apache::thrift::TException& ex) {
        }
    }
    else{
        std::shared_ptr<apache::thrift::transport::TSocket> socket(new apache::thrift::transport::TSocket("localhost", port));
        socket->setConnTimeout(1 * 1000);
...
void common_service_handler::get_service_list(std::set<service_info> & _return, const std::string& session_id){
    ...
    boost::filesystem::path p(work_dir);
    if (reg.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH))){
        if (reg[L"KeyPath"].exists() && reg[L"KeyPath"].is_string())
            p = reg[L"KeyPath"].wstring();
    }
        ...
        if (macho::windows::environment::is_winpe()){
            ...
            foreach(storage::volume::ptr v, vols){
                ...
                foreach(std::wstring access_path, v->access_paths()){
                    if (_is_drive_letter(access_path)){
                        dirs.push_back(access_path);
                    }
                }
            }
        }
        foreach(boost::filesystem::path dir, dirs){
            try{
                if (boost::filesystem::exists(dir / L"emulator\\qemu\\qemu-system-x86_64.exe") &&
                    boost::filesystem::exists(dir / L"emulator\\linux_launcher.img")){
                    info.id = saasame::transport::g_saasame_constants.LINUX_LAUNCHER_SERVICE;
                    info.path = boost::filesystem::path(dir / L"emulator\\linux_launcher.img").string();
                    _return.insert(info);
                    break;
                }
            }
    ...
    foreach(macho::windows::storage::disk::ptr d, disks){
        ...
        if (rd){
            ...
            if (rd->sector_read(((d->size()) >> 9) - 40, 1, data.get(), r)){
                ...
                _d.customized_id = macho::guid_(guid);
            }
            _d.__set_customized_id(_d.customized_id);
        }
        _return.insert(_d);
    }
}
...
void common_service_handler::get_host_general_info(physical_machine_info& _info){
    ...
    if (macho::windows::environment::is_winpe()){
        ...
        if (reg.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH)) && reg[_T("HostName")].exists()){
            _info.client_name = reg[_T("HostName")].string();
        }
    }
    ...
    _info.__set_system_default_ui_language(GetSystemDefaultUILanguage());
}
    ...
    if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
        ...
        while (pAdapter) {
            if (pAdapter->Type == MIB_IF_TYPE_ETHERNET || pAdapter->Type == 71){
                network_info _net;
                _net.adapter_name = macho::stringutils::convert_unicode_to_utf8(macho::stringutils::convert_ansi_to_unicode(pAdapter->AdapterName));
                _net.description = macho::stringutils::convert_unicode_to_utf8(macho::stringutils::convert_ansi_to_unicode(pAdapter->Description));
                _net.mac_address;
...
bool common_service_handler::verify_carrier(const std::string& carrier, const bool is_ssl){
    ...
    std::shared_ptr<apache::thrift::transport::TSSLSocket>                                 ssl_socket;
    std::shared_ptr<apache::thrift::transport::TTransport>                                 transport;
    std::shared_ptr<apache::thrift::protocol::TProtocol>                                   protocol;
    std::shared_ptr<apache::thrift::transport::TSSLSocketFactory>                          factory;
    std::shared_ptr<saasame::transport::common_serviceClient>   client;
    ...
    if (is_ssl)
    {
        ...
        if (boost::filesystem::exists(p / "server.key") && boost::filesystem::exists(p / "server.crt"))
        {
            try
            {
                ...
                ssl_socket = std::shared_ptr<apache::thrift::transport::TSSLSocket>(factory->createSocket(carrier, saasame::transport::g_saasame_constants.CARRIER_SERVICE_SSL_PORT));
    ...
    {
        socket = std::shared_ptr<apache::thrift::transport::TSocket>(new apache::thrift::transport::TSocket(carrier, saasame::transport::g_saasame_constants.CARRIER_SERVICE_PORT));
    ...
    if (transport){
        ...
        client = std::shared_ptr<saasame::transport::common_serviceClient>(new saasame::transport::common_serviceClient(protocol));
...
void common_service_handler::take_xrays(std::string& _return){
    ...
    if (zip_ptr){
        bool     has_data = false;
        bool     xray_file = false;
        macho::windows::registry reg;
        boost::filesystem::path p(macho::windows::environment::get_working_directory());
        if (reg.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH))){
            if (reg[L"KeyPath"].exists() && reg[L"KeyPath"].is_string())
                p = reg[L"KeyPath"].wstring();
            if (reg[L"XrayFile"].exists() && reg[L"XrayFile"].is_dword() && (DWORD)reg[L"XrayFile"] > 0)
                xray_file = true;
        }
        ...
        if (zip_ptr->close() && has_data){
            ...
            if (xray_file){
                boost::filesystem::path output_file_name = boost::filesystem::path(macho::windows::environment::get_working_directory()) / "xray.zip";
                boost::filesystem::remove(output_file_name);
                try{
                    std::ofstream ofs;
                    ofs.exceptions(std::ofstream::failbit | std::ofstream::badbit | std::ofstream::eofbit);
                    ofs.open(output_file_name.wstring(), std::ios::trunc | std::ios::out | std::ios::binary, _SH_DENYRW);
                    if (ofs.is_open()){
...
std::string common_service_handler::get_xray(int port, const std::string& host){
    ...
    if (boost::filesystem::exists(p / "server.key") && boost::filesystem::exists(p / "server.crt")){
        try
        {
            ...
            std::shared_ptr<apache::thrift::transport::AccessManager> accessManager(new MyAccessManager());
            factory->access(accessManager);
            std::shared_ptr<apache::thrift::transport::TSSLSocket> ssl_socket = std::shared_ptr<apache::thrift::transport::TSSLSocket>(factory->createSocket(host, port));
            ssl_socket->setConnTimeout(10 * 1000);
            transport = std::shared_ptr<apache::thrift::transport::TTransport>(new apache::thrift::transport::TBufferedTransport(ssl_socket));
        }
        catch (apache::thrift::TException& ex) {
...
bool common_service_handler::create_mutex(const std::string& session, const int16_t timeout){
    macho::windows::mutex mx(L"LockTransport");
    macho::windows::auto_lock lock(mx);
    macho::windows::registry reg(macho::windows::REGISTRY_CREATE);
    bool result = false;
    if (reg.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH))){
        LOG(LOG_LEVEL_RECORD, L"From %s", macho::stringutils::convert_utf8_to_unicode(session).c_str());
        if (!reg[L"LockTransport"].exists()){
            LOG(LOG_LEVEL_RECORD, L"Lock");
            boost::posix_time::ptime expired = boost::posix_time::second_clock::universal_time() + boost::posix_time::seconds(timeout > 30 ? timeout : 30 );
            reg[L"LockTransport"] = boost::posix_time::to_simple_string(expired);
            result = true;
        }
        else{
            boost::posix_time::ptime expired = boost::posix_time::time_from_string(reg[L"LockTransport"].string());
            if (expired < boost::posix_time::second_clock::universal_time()){
                LOG(LOG_LEVEL_RECORD, L"Lock Timeout");
                expired = boost::posix_time::second_clock::universal_time() + boost::posix_time::seconds(timeout > 30 ? timeout : 30);
                reg[L"LockTransport"] = boost::posix_time::to_simple_string(expired);
                result = true;
            }
        }
...
bool common_service_handler::delete_mutex(const std::string& session){
    ...
    if (reg.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH))){
        LOG(LOG_LEVEL_RECORD, L"From %s", macho::stringutils::convert_utf8_to_unicode(session).c_str());
        if (reg[L"LockTransport"].exists()){
            LOG(LOG_LEVEL_RECORD, L"Unlock");
            reg[L"LockTransport"].delete_value();
        }
    }
    return true;
}