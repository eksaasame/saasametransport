// irm_transporter.cpp : Defines the entry point for the console application.
//
...
std::shared_ptr<TThreadedServer>  g_transport_server;
std::shared_ptr<TThreadedServer>  g_http_server;
...
bool command_line_parser(po::variables_map &vm){
    ...
    config.add_options()
        ...
        ("ssl", "force using ssl connection")
        ;
    ...
    install.add_options()
        ...
        ("display", po::wvalue<std::wstring>()->default_value(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CARRIER_SERVICE_DISPLAY_NAME), saasame::transport::g_saasame_constants.CARRIER_SERVICE_DISPLAY_NAME), "service display name (optional, installation only)")
        ("description", po::wvalue<std::wstring>()->default_value(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CARRIER_SERVICE_DESCRIPTION), saasame::transport::g_saasame_constants.CARRIER_SERVICE_DESCRIPTION), "service description (optional, installation only)")
        ;
...
public:
    ...
    void worker()
    {
        ...
        _reverse_handler = std::shared_ptr<reverse_transport_handler>(new reverse_transport_handler());
        _transport_handler = std::shared_ptr<transport_service_handler>(new transport_service_handler(*_reverse_handler.get()));
      
        std::shared_ptr<TProcessor> reverse_processor(new reverse_transportProcessor(_reverse_handler));

        std::shared_ptr<TProcessor> transport_processor(new transport_serviceProcessor(_transport_handler));

        std::shared_ptr<TTransportFactory> transportFactory(new TBufferedTransportFactory());
        std::shared_ptr<TProtocolFactory> protocolFactory(new TBinaryProtocolFactory());
        std::shared_ptr<TServerTransport> transport_serverTransport(new TServerSocket(saasame::transport::g_saasame_constants.TRANSPORT_SERVICE_PORT));

        g_transport_server = std::shared_ptr<TThreadedServer>(new TThreadedServer(transport_processor,
            transport_serverTransport,
            transportFactory,
            protocolFactory));

        //http
        {
            std::shared_ptr<TServerTransport> httpTransport(new TServerSocket(saasame::transport::g_saasame_constants.TRANSPORT_SERVICE_HTTP_PORT));
            std::shared_ptr<TTransportFactory> httptransportFactory(new THttpServerTransportFactory());
            g_http_server = std::shared_ptr<TThreadedServer>(new TThreadedServer(reverse_processor,
                httpTransport,
        ...
        if (boost::filesystem::exists(p / "server.key") && boost::filesystem::exists(p / "server.crt"))
        {
            try
            {
                ...
                std::shared_ptr<TServerTransport> ssl_transport_serverTransport(new TSSLServerSocket(saasame::transport::g_saasame_constants.TRANSPORT_SERVICE_PORT, g_factory));
                g_transport_server = std::shared_ptr<TThreadedServer>(new TThreadedServer(transport_processor, ssl_transport_serverTransport, transportFactory, protocolFactory));
        ...
        if (g_transport_server)
            tg.create_thread(boost::bind(&TThreadedServer::serve, g_transport_server));
        if (g_http_server)
            tg.create_thread(boost::bind(&TThreadedServer::serve, g_http_server));

        tg.join_all();
        g_transport_server = NULL;
        g_http_server = NULL;
    ...
    bool stop()
    {
        ...
        if (_transport_handler)
            _transport_handler->terminate((std::string)_session);
...
private:
    std::shared_ptr<reverse_transport_handler>         _reverse_handler;
    std::shared_ptr<transport_service_handler>         _transport_handler;
...
bool setup(application::context& context)
{
    ...
    install.add_options()
        ...
        ("display", po::wvalue<std::wstring>()->default_value(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.TRANSPORTER_SERVICE_DISPLAY_NAME), saasame::transport::g_saasame_constants.TRANSPORTER_SERVICE_DISPLAY_NAME), "service display name (optional, installation only)")
        ("description", po::wvalue<std::wstring>()->default_value(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.TRANSPORTER_SERVICE_DESCRIPTION), saasame::transport::g_saasame_constants.TRANSPORTER_SERVICE_DESCRIPTION), "service description (optional, installation only)")
        ;
...
int _tmain(int argc, _TCHAR* argv[]){
    ...
    if (!macho::windows::environment::is_running_as_local_system()){
        ...
        {
            thrift_connect<transport_serviceClient> thrift(saasame::transport::g_saasame_constants.TRANSPORT_SERVICE_PORT, "localhost");
            if (thrift.open()){
                license_infos lics;
                thrift.client()->get_licenses(lics);
                foreach(license_info& lic, lics.licenses){
                    ...
                    std::cout << "Activated       : " << (lic.is_active ? "true" : "false") << std::endl << std::endl;
                }
                foreach(workload_history& w, lics.histories){
        ...
        {
#ifdef _DEBUG
            std::set<disk_info> ret;
            thrift_connect<launcher_serviceClient> _thrift(saasame::transport::g_saasame_constants.LAUNCHER_SERVICE_PORT, "192.168.31.22");
            std::set<service_info> _return;
            if (_thrift.open()){
                //thrift.client()->enumerate_disks(ret, enumerate_disk_filter_style::ALL_DISK);
                _thrift.client()->get_service_list(_return, "");
                if (_return.size()){
