...
#ifndef _NTFS_COMMON_
#define _NTFS_COMMON_
...
typedef struct _BIOS_PARAMETER_BLOCK // DOS 3.31 BPB
{
    WORD BytesPerSector;
    BYTE SectorsPerCluster;
    WORD ReservedSectors;
    BYTE NumberOfFATs; // always 0 for NTFS
    WORD RootEntries;  // always 0 for NTFS
    WORD NumberOfSectors; // not used by NTFS
    BYTE MediaDescriptor;
    WORD SectorsPerFAT; // always 0 for NTFS
    WORD SectorsPerTrack;
    WORD TracksPerCylinder;
    DWORD HiddenSectors;
    DWORD BigNumberOfSectors; // not used by NTFS
}BIOS_PARAMETER_BLOCK, *PBIOS_PARAMETER_BLOCK;

typedef struct _NTFS_EXTENDED_BIOS_PARAMETER_BLOCK
{
    BYTE                   PhysicalDriveNumber;
    BYTE                   Flags; // (identical to DOS 3.4 EBPB)
    BYTE                   ExtendedBootSignature; // (0x80 aka "8.0")
    BYTE                   Reserved;
    ULONGLONG              TotalSectors;
    ULONGLONG              MFTFirstClusterNumber;
    ULONGLONG              MFTMirrorFirstClusterNumber;
    ...
    DWORD                  Checksum;
}NTFS_EXTENDED_BIOS_PARAMETER_BLOCK, *PNTFS_EXTENDED_BIOS_PARAMETER_BLOCK;

typedef struct _PARTITION_BOOT_SECTOR
{
    BYTE                               JumpCode[3];
    BYTE                               OEM_Identifier[8];
    BIOS_PARAMETER_BLOCK               BPB;
    NTFS_EXTENDED_BIOS_PARAMETER_BLOCK ExtendedBPB;
    BYTE                               BootstrapCode[426];
    WORD                               EndofSectorMarker;
}PARTITION_BOOT_SECTOR, *PPARTITION_BOOT_SECTOR;
...
typedef struct _ATTR_ATTRIBUTE_LIST
{
...
} ATTR_ATTRIBUTE_LIST, *PATTR_ATTRIBUTE_LIST;
...
typedef struct _ATTR_VOLUME_INFORMATION
{
...
} ATTR_VOLUME_INFORMATION, *PATTR_VOLUME_INFORMATION;
