...
void xfs_volume::scanfunc(__uint32_t block, int level, xfs_agf_t *agf, io_range::vtr &excludes){
    int _level = level - 1;
    size_t buff_size = sbp.sb_blocksize;
    std::auto_ptr<BYTE> buff = std::auto_ptr<BYTE>(new BYTE[buff_size]);
        ...
        if (bb_magic == XFS_ABTB_MAGIC || bb_magic == XFS_ABTB_CRC_MAGIC){
            if (0 == _level){
                ...
                for (int i = 0; i < be16_to_cpu(pblock->bb_numrecs); i++){
                    ...
                    excludes.push_back(io_range(_offset + (startblock * sbp.sb_blocksize), (blockcount * sbp.sb_blocksize)));
                }
            }
            else{
...
io_range::vtr xfs_volume::file_system_ranges(){
            ...
            if (_sbp.sb_magicnum == XFS_SB_MAGIC){
                ...
                if (_agf.agf_magicnum == XFS_AGF_MAGIC){                 
                    ...
                    if (_agf.agf_flcount){
                        ...
                        for (;;) {
                            excludes.push_back(io_range(_offset + ((__uint64_t)be32_to_cpu(freelist[i]) * sbp.sb_blocksize), sbp.sb_blocksize));
    ...
    std::sort(excludes.begin(), excludes.end(), io_range::compare());
    foreach(io_range &ex, excludes){
        if (ex.start > next_start)
            ranges.push_back(io_range(next_start, ex.start - next_start));
        next_start = ex.start + ex.length;
    }
    if (next_start < total_size){
        ranges.push_back(io_range(next_start, total_size - next_start));
    }
    return ranges;
}
