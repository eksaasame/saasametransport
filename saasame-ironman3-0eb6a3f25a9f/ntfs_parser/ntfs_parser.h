...
public:
    ...
    std::wstring file_name(){ return name; }
private:
...
public:
    ...
    file_record::ptr   get_file_record(ULONGLONG fileRef, DWORD mask = (MASK_INDEX_ROOT | MASK_INDEX_ALLOCATION | MASK_FILE_NAME | MASK_DATA)){
        return get_file_record(fileRef, L"", mask);
    }
    ...
    file_record::vtr   find_sub_entry(const std::wregex filter, const file_record::ptr dir = NULL);
...
private:
    ...
    file_record::ptr   get_file_record(ULONGLONG fileRef, std::wstring name, DWORD mask = (MASK_INDEX_ROOT | MASK_INDEX_ALLOCATION | MASK_FILE_NAME | MASK_DATA));
    bool visit_index_block(const file_record::ptr dir, const ULONGLONG vcn, const std::wstring& file_name, ULONGLONG &fileRef);
    bool visit_index_block(const file_record::ptr dir, const ULONGLONG vcn, const std::wregex filter, file_record::vtr& result);
