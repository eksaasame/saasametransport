...
struct clone_disk_rw{
    typedef std::map< std::wstring, clone_disk_rw>map;
    clone_disk_rw() : size(0){}
    clone_disk_rw(universal_disk_rw::ptr _rw, uint64_t _size) : rw(_rw), size(_size){}
...
public:
    ...
    static bool can_be_shrink_by_resize2fs(universal_disk_rw::ptr rw, size_t size);
    static changed_area::vtr get_clone_data_range(int disk_number, bool file_system_filter_enable = true);
    static changed_area::vtr get_clone_data_range(const clone_disk_rw& rw, bool file_system_filter_enable = true, bool no_minimize_size = false);
    static std::map<std::wstring, changed_area::vtr> get_clone_data_range(const clone_disk_rw::map& rws, bool file_system_filter_enable = true, bool no_minimize_size = false);
private:
    ...
    static changed_area::vtr final_changed_area(changed_area::vtr& src, changed_area::vtr& excludes, uint64_t max_size, bool no_minimize_size);
