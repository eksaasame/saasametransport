...
changed_area::vtr clone_disk::get_clone_data_range(int disk_number, bool file_system_filter_enable){
            ...
            foreach(macho::windows::storage::partition::ptr p, parts){
                    ...
                    )
                {
                    if (file_system_filter_enable){
                        ...
                        if (vol){
                            io_range::vtr ranges = vol->file_system_ranges();
    ...
    foreach(io_range& r, lvm_system_ranges[disk_rw->path()]){
        changeds.push_back(changed_area(r.start, r.length));
    }
    ...
    return final_changed_area(changeds, excluded, max_size, false);
}
...
changed_area::vtr clone_disk::get_clone_data_range(const clone_disk_rw& rw, bool file_system_filter_enable, bool no_minimize_size){
    clone_disk_rw::map rws;
    rws [rw.rw->path()]= rw;
    std::map<std::wstring, changed_area::vtr> changed = get_clone_data_range(rws, file_system_filter_enable, no_minimize_size);
    return changed[rw.rw->path()];
}

std::map<std::wstring, changed_area::vtr> clone_disk::get_clone_data_range(const clone_disk_rw::map& rws, bool file_system_filter_enable, bool no_minimize_size){
    uint64_t max_size = 1024 * 1024 * 8;
    std::map<std::wstring, changed_area::vtr> changed_areas;
    std::map<std::wstring, changed_area::vtr> excluded_areas;
    logical_volume_manager lvm;
    foreach(const clone_disk_rw::map::value_type& _clone_disk_rw, rws){
        bool result = false;
        universal_disk_rw::ptr rw = _clone_disk_rw.second.rw;
        ...
        if (result = rw->sector_read(0, GPT_PART_SIZE_SECTORS, data_buf.get(), number_of_sectors_read)){
            ...
            if (part_table->Signature != 0xAA55){
                linuxfs::volume::ptr v = file_system_filter_enable ? linuxfs::volume::get(rw, 0,_clone_disk_rw.second.size) : NULL;
                if (v){
                    io_range::vtr ranges = v->file_system_ranges();
                    foreach(io_range& r, ranges){
                    ...
                    changed.push_back(changed_area(0, _clone_disk_rw.second.size));
                }	
            }
            ...
            else
            {
                for (int index = 0; index < 4; index++)
                {
                    ...
                    else if (part_table->PartitionRecord[index].SizeInLBA){
                        changed.push_back(changed_area(start, ((uint64_t)part_table->PartitionRecord[index].SizeInLBA * rw->sector_size())));
                    }
                }
            }
        ...
        std::sort(excluded.begin(), excluded.end(), changed_area::compare());
        size_t length = 1024 * 1024;
        if ((!changed.empty()) && changed[0].start < length)
            length = changed[0].start;
        changed.insert(changed.begin(), changed_area(0, length));
        changed_areas[rw->path()] = changed;
        excluded_areas[rw->path()] = excluded;
    }
    io_range::map lvm_system_ranges = lvm.get_system_ranges();
    foreach(io_range::map::value_type &v, lvm_system_ranges){
        foreach(io_range& r, v.second){
            changed_areas[v.first].push_back(changed_area(r.start, r.length));
        }
    }
    typedef std::map<std::wstring, changed_area::vtr> changed_areas_map;
    changed_areas_map totle_changed_areas;
    foreach(changed_areas_map::value_type &v, changed_areas){
        totle_changed_areas[v.first] = final_changed_area(v.second, excluded_areas[v.first], max_size, no_minimize_size);
    }
    return totle_changed_areas;
}

changed_area::vtr clone_disk::final_changed_area(changed_area::vtr& src, changed_area::vtr& excludes, uint64_t max_size, bool no_minimize_size){
    ...
    if (rw->sector_read(0, GPT_PART_SIZE_SECTORS, data_buf.get(), number_of_sectors_read)){
        ...
        if (part_table->Signature == 0xAA55){
            if (part_table->PartitionRecord[0].PartitionType == 0xEE){  //GPT
                ...
                for (int i = 0; i < gpt_hdr->NumberOfPartitionEntries; i++){
                    macho::guid_ g(gpt_part[i].PartitionTypeGuid);
...
bool clone_disk::can_be_shrink_by_resize2fs(universal_disk_rw::ptr rw, size_t size){
    bool result = false;
    uint64_t boundary = 0;
    uint64_t part_start = 0;
    uint64_t part_length = 0;
    bool supported_partition_type = true;
    std::auto_ptr<BYTE> data_buf(new BYTE[GPT_PART_SIZE_SECTORS * SECTOR_SIZE]);
    uint32_t number_of_sectors_read = 0;
    PLEGACY_MBR part_table;
    if (result = rw->sector_read(0, GPT_PART_SIZE_SECTORS, data_buf.get(), number_of_sectors_read)){
        ...
        if (part_table->Signature == 0xAA55){
            if (part_table->PartitionRecord[0].PartitionType == 0xEE) //GPT
            {
                ...
                for (int i = 0; i < gpt_hdr->NumberOfPartitionEntries; i++)
                {
                    ...
                    if (boundary < b){
                        boundary = b;
                        part_start = (((uint64_t)gpt_part[i].StartingLBA) * rw->sector_size());
                        part_length = ((gpt_part[i].EndingLBA - gpt_part[i].StartingLBA + 1) * rw->sector_size());
                        ...
                        if (partition_type == macho::guid_("0657FD6D-A4AB-43C4-84E5-0933C84B4F4F") || // Linux SWAP partition
                            ...
                            partition_type == macho::guid_("e3c9e316-0b5c-4db8-817d-f92df00215ae"))  // Microsoft reserved partition
                        {
                            supported_partition_type = false;
                        }
                        else if (partition_type == macho::guid_("af9b60a0-1431-4f62-bc68-3311714a69ad") || // LDM data partition
                            partition_type == macho::guid_("5808c8aa-7e8f-42e0-85d2-e1e90434cfb3"))  // LDM metadata partition
                        {
            ...
            if (boundary > size){
                if (supported_partition_type && part_start < size && part_length > 0){
