...
void soap_set_logging_inbound(struct soap *soap, void *ss)
...
void soap_set_logging_outbound(struct soap *soap, void *ss)
{ struct logging_data *data = (struct logging_data*)soap_lookup_plugin(soap, logging_id);
  if (data)
    data->outbound = ss;
}
...
{ struct logging_data *data = (struct logging_data*)soap_lookup_plugin(soap, logging_id);
  ...
  data->stat_recv += res;
  if (data->inbound)
    static_cast<std::stringstream*>(data->inbound)->write(buf, res);
  return res;
}
...
{ struct logging_data *data = (struct logging_data*)soap_lookup_plugin(soap, logging_id);
  ...
  data->stat_sent += len;
  if (data->outbound)
    static_cast<std::stringstream*>(data->outbound)->write(buf, len);
