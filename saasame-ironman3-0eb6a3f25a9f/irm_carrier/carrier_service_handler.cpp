...
void carrier_service_handler::clean_timeout_image_instances()
{
    if (_instance_cs.trylock()){
        ...
        image_instance::vtr::iterator p = _image_instances.begin();
        while(p != _image_instances.end())
        {
    ...
    try
    {
        ...
        if (op)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (image_obj)
                    {
                        macho::windows::auto_lock lock(image_obj->cs);
                        ...
                        if (jobs.size())
                        {
                            ...
                            foreach(repeat_job::ptr j, jobs)
                            {
                                ...
                                foreach(macho::removeable_job::map::value_type& _j, _scheduled){
                                    ...
                                    if (r->image_name() == name && r->connection_id() == j->connection_id()){
                                        found = true;
                                        break;
                                    }
                                }                               
                                if (!found){
                                    j->register_is_image_uploaded_callback_function(boost::bind(&carrier_service_handler::is_image_uploaded, this, _1, _2));
        ...
        if (parent.length()){
            ...
            if (!(ERROR_SUCCESS == ( err = win_vhdx_mgmt::get_virtual_disk_information(output_parent_file.wstring(), disk_info)) &&
                ...
                LOG(LOG_LEVEL_ERROR, L"Failed to create the virtual disk file (%s).", output_file.wstring().c_str()); 
            }                
        }
        else{
...
bool carrier_service_handler::close(const std::string& session_id, const std::string& image_id, const bool is_cancel)
{
    ...
    //get image object by image id
    {
        ...
        if (it != _image_instances.end())
        {
            ...
            (*it)->updated_time = update_time;
            {
                macho::windows::auto_lock lock((*it)->cs);
                if ((*it)->outfile)
                {
                    irm_transport_image *image = ((*it)->outfile.get());
                    image_name = (*it)->image_name;
                    LOG(LOG_LEVEL_RECORD, L"Close image(%s) with cancel flag(%s).", macho::stringutils::convert_utf8_to_unicode((*it)->image_name).c_str(), (is_cancel ? L"true" : L"false"));
...
void carrier_service_handler::stop_all()
{
    ...
    {
        macho::windows::auto_lock lock(image->cs);
        if (image->outfile)
        {
            ...
            if (fs::is_regular_file(dir_iter->status())){
                if (dir_iter->path().extension() == JOB_EXTENSION){
                    ...
                    if (job->state() & mgmt_job_state::job_state_finished){
                        ...
                        job->register_is_image_uploaded_callback_function(boost::bind(&carrier_service_handler::is_image_uploaded, this, _1, _2));
...
bool carrier_service_handler::is_image_uploaded(std::string parent, std::string connection_id)
{
    bool result = true;
    if (parent.length())
    {
        ...
        if (_scheduler.has_group_jobs(wp))
        {
            ...
            {
                repeat_job* r = dynamic_cast<repeat_job*>((*j).second.get());
                if (r && 0 == _wcsicmp(j->second->group().c_str(), wp.c_str()) && r->connection_id() == connection_id)
                {
                    result = false;
                    if (r->state() & mgmt_job_state::job_state_finished)
                    {
...
bool carrier_service_handler::remove_connection(const std::string& session_id, const std::string& connection_id){
#ifndef IRM_TRANSPORTER                 
    macho::windows::auto_lock lock(_lock);
    macho::removeable_job::map::iterator j = _scheduled.begin();
    while (j != _scheduled.end()){
...
#endif
    return common_connection_service_handler::remove_connection(session_id, connection_id);
}

bool carrier_service_handler::send_checksum_info(const std::string& session_id, const std::string& connection_id, const std::string& file_name, const std::string& data){
    ...
    if (connections_map.count(connection_id)){
        ...
        case  saasame::transport::connection_type::LOCAL_FOLDER:{
            std::string path = conn.detail.local.path;
            if (conn.options.count("folder"))
                path = conn.options["folder"];
            op = irm_imagex_op::ptr(new irm_imagex_local_op(path, _working_path, _data_path, 5, "c", reserved_space));
            break;
        }
        ...
        case saasame::transport::connection_type::WEBDAV_EX:
            ...
            if (dav){
                op = irm_imagex_op::ptr(new irm_imagex_webdav_op(dav, _working_path, _data_path, 5, "c"));
            }
            break;
        };
        if (op){
            macho::windows::lock_able::vtr locks_ptr = op->get_lock_objects(macho::stringutils::convert_utf8_to_unicode(file_name));
            macho::windows::auto_locks block_file_locks(locks_ptr);
            return op->write_file(file_name, std::stringstream(data));
        }
    }
    else{
        invalid_operation err;
        err.what_op = error_codes::SAASAME_E_INVALID_ARG;
        err.why = boost::str(boost::format("Unknown connection id(%1%).") % connection_id);
        LOG(LOG_LEVEL_ERROR, L"%s", macho::stringutils::convert_utf8_to_unicode(err.why).c_str());
        throw err;
    }
    return false;
...
void carrier_service_handler::receive_checksum_result(std::string& _return, const std::string& session_id, const std::string& connection_id, const std::string& file_name){
    boost::uintmax_t reserved_space = (((boost::uintmax_t)2) * 1024 * 1024 * 1024);
    if (connections_map.count(connection_id)){
        connection conn = connections_map[connection_id];
        irm_imagex_op::ptr op;
        ...
        if (op){
            ...
            if (op->read_file(file_name, buffer))
                _return = buffer.str();
        }
    }
    else{
