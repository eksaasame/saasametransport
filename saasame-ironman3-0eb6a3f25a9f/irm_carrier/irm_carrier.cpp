...
std::shared_ptr<TThreadedServer>  g_ssl_server;
...
public:
    ...
    void worker()
    {
        ...
        int port = saasame::transport::g_saasame_constants.CARRIER_SERVICE_PORT;
        ...
        if (reg.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH))){
            if (reg[L"ConcurrentCarrierJobNumber"].exists() && (DWORD)reg[L"ConcurrentCarrierJobNumber"] > 0)
                workers = (int)reg[L"ConcurrentCarrierJobNumber"];
            if (reg[L"KeyPath"].exists() && reg[L"KeyPath"].is_string())
                p = reg[L"KeyPath"].wstring();
            reg.close();
        }

        _handler = std::shared_ptr<carrier_service_handler>(new carrier_service_handler(vm["session"].as<std::wstring>(), workers));
        std::shared_ptr<TProcessor> processor(new carrier_serviceProcessor(_handler));
        ...
        //http
        {
            std::shared_ptr<TServerTransport> httpTransport(new TServerSocket(saasame::transport::g_saasame_constants.CARRIER_SERVICE_HTTP_PORT));
            std::shared_ptr<TTransportFactory> httptransportFactory(new THttpServerTransportFactory());
            g_http_server = std::shared_ptr<TThreadedServer>(new TThreadedServer(processor,
                httpTransport,
        ...
        if (boost::filesystem::exists(p / "server.key") && boost::filesystem::exists(p / "server.crt"))
        {
            try
            {
                port = saasame::transport::g_saasame_constants.CARRIER_SERVICE_SSL_PORT;
                ...
                g_ssl_server = std::shared_ptr<TThreadedServer>(new TThreadedServer(processor, ssl_serverTransport, transportFactory, protocolFactory));
        ...
        if (g_ssl_server)
        {
            ...
            tg.create_thread(boost::bind(&TThreadedServer::serve, g_ssl_server));
        }
        else
        {
            if (g_server)
...
private:
    std::shared_ptr<carrier_service_handler>   _handler;
...
#ifdef _DEBUG
                std::shared_ptr<carrier_service_handler> handler(new carrier_service_handler(vm["session"].as<std::wstring>()));
                std::shared_ptr<TProcessor> processor(new carrier_serviceProcessor(handler));
                std::shared_ptr<TServerTransport> serverTransport(new TServerSocket(saasame::transport::g_saasame_constants.CARRIER_SERVICE_PORT));
                std::shared_ptr<TTransportFactory> transportFactory(new TBufferedTransportFactory());
                std::shared_ptr<TProtocolFactory> protocolFactory(new TBinaryProtocolFactory());
                g_server = std::shared_ptr<TThreadedServer>(new TThreadedServer(processor,
                    serverTransport,
