...
$( document ).ready(function() {
	...
	function QueryReplicaInformation(){
		$.ajax({
			...
			success:function(jso)
			{
				...
				if (jso.WINPE_JOB == 'N'){$("#REBOOT_SWITCH").remove();}
			...
			success:function(jso)
			{
				if (jso != false)
				{
					$('#ConnectionTable > tbody').append(
						'<tr><th colspan="2"><?php echo _("Recover Kit Information"); ?></th></tr>\
						 ...
						 <tr><td width="165px"><?php echo _("Connection Type"); ?></td>	<td>'+jso.CONN_DATA.CONN_TYPE+'</td></tr>\
						 <tr><td width="165px"><?php echo _("Connection Folder"); ?></td><td>'+jso.CONN_DATA.detail.local.path+'</td></tr>\
						 <tr><td width="165px"><?php echo _("webDAV Path"); ?></td><td>'+jso.CONN_DATA.detail.remote.path+'</td></tr>\
	...
	function BeginToRunRecoverKitServiceAsync(){
		$.ajax({
			type: 'POST',
			dataType:'JSON',
			url: '_include/_exec/mgmt_service.php',
			data:{
				 'ACTION'      :'BeginToRunRecoverKitServiceAsync',
				 ...
				 'AUTO_REBOOT' :$('#AUTO_REBOOT:checkbox:checked').val()
			},
			success:function(jso)
			{
	...
	$(function(){
		...
		$("#ServiceSubmitAndRun").one("click" ,function(){
			...
			BeginToRunRecoverKitServiceAsync();
		})
	});
});
</script>
				...
				<table id="TriggerSync">
					<tbody>
						<div class='form-group' style='float:right;'>
							<span id="REBOOT_SWITCH"><?php echo _("Reboot after completed:"); ?> <input id="AUTO_REBOOT" data-toggle="toggle" data-width="60" data-style="slow" type="checkbox"></span>
							<span class="super_hidden"><select id="SET_CONVERT_TYPE" class="selectpicker" data-width="110px"></select></span>
