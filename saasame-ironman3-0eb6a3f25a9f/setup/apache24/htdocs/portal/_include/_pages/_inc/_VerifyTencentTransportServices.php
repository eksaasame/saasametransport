...
<script>
				...
				if (jso != false)
				{
					$('#InstanceDetailTable > tbody').append(
						'<tr><td>Name</td><td>'+jso.InstanceName+'</td></tr>\
						 ...
						 <tr><td>Internet Address</td><td><input type="text" id="OtherIpAddress"></input></td></tr>\
					...
					DetermineSegment('EditTencentTransportServices');
	...
	function InitializeNewServices(service){
		$.ajax({
			...
			data:{
				 ...
				 'HOST_UUID' :'<?php echo $HOST_UUID.'|'.$HOST_REGN; ?>'+'|Tencent',
				 'SERV_ADDR' :transport_addr,
				 'SYST_TYPE' :'WINDOWS',
				 'SELT_SERV' :service,
				 'MGMT_ADDR' :document.domain,
	...
	/* Initialize New Folder Connection */
	function InitializeNewFolderConnection(SERV_UUID){
		$.ajax({
			type: 'POST',
			dataType:'JSON',
			url: '_include/_exec/mgmt_service.php',
	...
	$(function(){
		...
		$("#CheckAndSubmit").click(function(){
			$("#LoadingOverLay").addClass("GrayOverlay GearLoading");
			$('#CheckAndSubmit').prop('disabled', true);
			if ($URL_SEGMENT == 'VerifyTencentTransportServices')
			{		
