...
<script>
		...
		$.ajax({
			...
			success:function(jso)
			{
				...
				if (jso != false)
				{
					$.each(jso, function(key,value)
					{
						...
						$('#InstanceDetailTable > tbody').append(
							'<tr><td><?php echo _("Name"); ?></td><td>'+value.name+'</td></tr>\
							 ...
							 <tr><td><?php echo _("Cluster IP"); ?></td><td>-</td></tr>\
							 ...
							 <tr><td><?php echo _("Spice Port"); ?></td><td>'+value.spice_port+'</td></tr>\
					...
					DetermineSegment('EditCtyunTransportServices');
				}
				else
				{
					$('#InstanceDetailTable > tbody').append(
	...
	$(function(){
		...
		$("#CheckAndSubmit").click(function(){
			$("#LoadingOverLay").addClass("GrayOverlay GearLoading");
			$('#CheckAndSubmit').prop('disabled', true);
			if (URL_SEGMENT == 'VerifyCtyunTransportServices')
			{		
