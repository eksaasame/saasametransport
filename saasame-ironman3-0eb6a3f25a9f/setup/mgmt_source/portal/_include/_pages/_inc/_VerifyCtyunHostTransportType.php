	...
	function QuerySelectedHostInformation(){
		$.ajax({
			...
			success:function(jso)
			{
					...
					$.each(jso, function(key,value)
					{
						...
						$('#InstanceDetailTable > tbody').append(
							'<tr><td><?php echo _("Name"); ?></td><td>'+value.name+'</td></tr>\
							 <tr><td><?php echo _("Cluster Name"); ?></td><td>'+value['OS-EXT-SRV-ATTR:host']+'</td></tr>\
							 <tr><td><?php echo _("Instance Id"); ?></td><td>'+InstanceId+'</td></tr>\
							 <tr><td><?php echo _("IP"); ?></td><td id="fixed_addr">'+fixed_addr+'</td></tr>\
							 <tr><td><?php echo _("Floating IP"); ?></td><td id="floating_addr">'+floating_addr+'</td></tr>\
				...
				{
					$('#InstanceDetailTable > tbody').append(
						'<tr><td class="TextCenter">-</td><td colspan="2"><?php echo _("Failed to query host information."); ?></td></tr>'
					);
				}
			...
			success:function(jso)
			{
				if (jso != false)
				{
					$.each(jso, function(key,value)
					{
						...
						var mapObj = {UnknownVendorType:"On-Premises",Ctyun:"天翼云"};
						HOST_TYPE = HOST_TYPE.replace(/UnknownVendorType|Ctyun/gi, function(matched){return mapObj[matched];});	
						
						/* SELECT HOME SERVER ONLY OPENSTACK TYPE */
						if (this.HOST_TYPE == 'Ctyun')
						{
