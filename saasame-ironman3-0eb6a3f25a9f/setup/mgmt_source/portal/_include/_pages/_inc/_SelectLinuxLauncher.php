			...
			success:function(jso)
			{
				...
				if (jso != false)
				{
					...
					$.each(jso.servers, function(key,value)
					{
						...
						(
							'<tr>\
								 <td width="60px" class="TextCenter"><input type="radio" id="LAUNCHER_UUID" name="LAUNCHER_UUID" value="'+value.id+'" '+disable_status+'/></td>\
								 <td width="385px">'+value.name+'</td>\
								 <td width="285px">'+fixed_addr+'</td>\
								 <td width="285px">'+floating_addr+'</td>\
								 <td width="185px">'+value.status+'</td>\
							</tr>'
						);
						count++;						
					});
				...
				{
					console.warn(jso.code);
					console.error(jso);
					$('#InstanceTable > tbody').append(
						'<tr>\
							<td class="TextCenter">-</td>\
							<td colspan="4">No Server found on selected connection.</td>\
						</tr>');
				}
	...
	function SelectedInstance(){
		$.ajax({
			...
			success:function(jso)
			{
				if (jso.code == true)
				{
					window.location.href = "VerifyLinuxLauncherService";
				}
				else
				{
			...
			<div id='form_block_wizard'>
				<table id="InstanceTable">
					...
					<tr>
						...
						<th width="385px"><?php echo _("Name"); ?></th>
						<th width="285px"><?php echo _("IP"); ?></th>
						<th width="285px"><?php echo _("Floating IP"); ?></th>
						<th width="185px"><?php echo _("Status"); ?></th>
					</tr>
					</thead>
				
					<tbody>
						<tr id='LoadingTable'><td height='37px' colspan='5'><div class="DefaultLoading"></div></td></tr>
					</tbody>
				</table>
			</div>
