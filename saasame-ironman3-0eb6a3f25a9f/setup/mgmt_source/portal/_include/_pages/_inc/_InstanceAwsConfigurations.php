<script>
	...
	$(document).one("ajaxStop", function() {
		$('#ServiceConfiguration').prop('disabled', false);
		$('#ServiceConfiguration').removeClass('btn-default').addClass('btn-primary');
		...
		$.ajax({
			...
			success:function(jso)
			{
				if (jso != false)
				{
					...
					{
						$('#FLAVOR_LIST').append(new Option(key+' (CPU:'+value.vCPU+', Memory:'+value.Memory.replace(/\s/g, '')+')', key, true, false));
		...
		$.ajax({
			...
			success:function(jso)
			{
				if (jso != false)
				{
					...
					$.each(jso, function(key,value)
					{
						$('#NETWORK_LIST').append(new Option(value.CidrBlock, value.SubnetId+'|'+value.VpcId+'|'+value.CidrBlock, true, false));											
					});
					$('.selectpicker').selectpicker('refresh');	
	...
	function ListDataModeInstance(){
		$.ajax({
			...
			data:{
				 ...
				 'CLUSTER_UUID':'<?php echo $_SESSION['CLUSTER_UUID']; ?>',
				 'SELECT_ZONE' :'<?php echo $_SESSION['SERV_REGN']; ?>',
				 'TRANSPORT_ID':'<?php echo $_SESSION['SERV_UUID']; ?>'
			},
			beforeSend: function() {
			...
			success:function(jso)
			{
				...
				if (jso != false)
				{
					$.each(jso, function(key,value)
					{
						...
						$('#DATAMODE_INSTANCE').append(new Option(InstanceName, InstanceInfo, true, false));						
					});					
				}
	...
	$(function(){
		...
		$("#BackToServiceList").click(function(){
			...
			window.location.href = "SelectListEbsSnapshot";
		})
		
		$("#ServiceConfiguration").click(function(){
			PrivateAddrCheck();
			//ServiceConfiguration();
		})
	});
		...
		<div class="page">
			...
			<div id='title_block_wizard'>
				<div class='btn-toolbar'>
					...
					<button id='ServiceConfiguration' 		class='btn btn-default pull-right btn-lg' disabled><?php echo _("Next"); ?></button>					
				</div>
			</div>
		</div>			
