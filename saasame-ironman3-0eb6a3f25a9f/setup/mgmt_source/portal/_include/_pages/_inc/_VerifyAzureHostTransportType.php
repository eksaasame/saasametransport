...
<script>
			...
			success:function(jso)
			{
				if (jso != false)
				{
					$.each(jso, function(key,value)
					{
						...
						var mapObj = {UnknownVendorType:"On-Premises",AzureBlob:"Azure",Azure:"Azure"};
						HOST_TYPE = HOST_TYPE.replace(/UnknownVendorType|AzureBlob/gi, function(matched){return mapObj[matched];});	
						
						/* SELECT HOME SERVER ONLY Azure TYPE */
						if (this.HOST_TYPE == 'Azure')
						{						
	...
	$(function(){
		...
		$("#CheckAndSubmit").click(function(){
			...
			$('#CheckAndSubmit').prop('disabled', true);
			$('#CheckAndSubmit').removeClass('btn-primary').addClass('btn-default');
			if (url_segment == 'VerifyAzureHostTransportType')
			{
