<script>
	...
	function QueryVolumeDetailInfo(SelectKey,DiskId){
		$.ajax({
			type: 'POST',
			dataType:'JSON',
	...
	function ListAvailableSnapshot(SelectKey,DiskId){
		$.ajax({
			type: 'POST',
			dataType:'JSON',
			url: '_include/_exec/mgmt_ctyun.php',
			...
			success:function(jso)
			{
				...
				{	
					...
					$.each(jso, function(key,value)
					{
						...
						{
							$('#'+SelectKey+'').append(new Option(value['description'], value['id'], false, false));
							SNAPSHOT_STATUS = true;
						}
						else
						{
							$('#'+SelectKey+'').append(new Option(value['description']+' ('+value['progress']+')', value['id'], false, false));
							$("option[value='"+value['id']+"']").attr("disabled", "disabled");
						}						
					});
	...
	function SelectServiceSnapshot(){
		...
		$.ajax({
			...
			success:function(jso)
			{
				if (jso.Code == true)
				{
					window.location.href = "InstanceCtyunConfigurations";
				}
				else
				{
