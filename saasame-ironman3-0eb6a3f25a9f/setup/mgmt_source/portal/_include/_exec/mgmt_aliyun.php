...
{
	$URL = 'http://127.0.0.1:8080/restful/AliyunWebServices';
...
switch ($SelectAction)
{
	...
	if ($_REQUEST['ACCESS_KEY'] != '' and $_REQUEST['SECRET_KEY'] != '')
	{
		...
		{
			$response = array('Msg' => '','Code' => true);
		}
		else
		{
			$response = array('Msg' => _('Cloud connection could not be verified.<br>Please check that the information provided is correct.'),'Code' => false);
		}
	}
	else
	{
		$response = array('Msg' => _('Please fill in all of the required fields.'),'Code' => false);
	}
