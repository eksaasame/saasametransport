...
typedef enum TRACE_LOG_LEVEL {
	...
	LOG_LEVEL_INTERNAL_ERROR   = 7L,
	LOG_LEVEL_INTERNAL_WARNING = 8L,
	LOG_LEVEL_INTERNAL_NOTICE  = 9L,
    LOG_LEVEL_TRACE            = 10L,
...
stdstring& trace_log_header( TRACE_LOG_LEVEL level ){                            
	...
	case LOG_LEVEL_INTERNAL_ERROR:
		ch = _T('E');    break;
	case LOG_LEVEL_WARNING:
	case LOG_LEVEL_INTERNAL_WARNING:
		ch = _T('W');    break;
	case LOG_LEVEL_NOTICE:
	case LOG_LEVEL_INTERNAL_NOTICE:
		ch = _T('N');    break;
...
#define FUN_TRACE_MSG(x)                                                                          \
        auto_log logFun(boost::str( boost::wformat(_T("%1%(%2%) : %3% %4%") )%_T(__FILE__)% __LINE__ %_T(__FUNCTION__)%x ) );
#else
...
#define FUN_TRACE_MSG(x)                                                                                       \
        auto_log logFun(boost::str( boost::wformat(_T("%1% %2%") )%_T(__FUNCTION__)%x ) );
#endif
