...
public:
    ...
    void worker()
    {
        ...
        _handler = std::shared_ptr<virtual_packer_service_handler>(new virtual_packer_service_handler(vm["session"].as<std::wstring>()));
        std::shared_ptr<TProcessor> processor(new virtual_packer_serviceProcessor(_handler));
...
private:
    std::shared_ptr<virtual_packer_service_handler> _handler;
...
int _tmain(int argc, _TCHAR* argv[]){
    ...
    if (command_line_parser(vm)){
        set_log_level((TRACE_LOG_LEVEL)vm["level"].as<int>());
#if _DEBUG
        set_log_level(TRACE_LOG_LEVEL::LOG_LEVEL_INTERNAL);
#endif
...
#ifdef _DEBUG
                int port = saasame::transport::g_saasame_constants.VIRTUAL_PACKER_SERVICE_PORT;
                std::shared_ptr<virtual_packer_service_handler> handler(new virtual_packer_service_handler(vm["session"].as<std::wstring>()));
                std::shared_ptr<TProcessor> processor(new virtual_packer_serviceProcessor(handler));
                std::shared_ptr<TServerTransport> serverTransport(new TServerSocket(port));
                std::shared_ptr<TTransportFactory> transportFactory(new TBufferedTransportFactory());
                std::shared_ptr<TProtocolFactory> protocolFactory(new TBinaryProtocolFactory());
                macho::windows::registry reg;
