...
bool command_line_parser(po::variables_map &vm){
    ...
    install.add_options()
        ...
        ("display", po::wvalue<std::wstring>()->default_value(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.SCHEDULER_SERVICE_DISPLAY_NAME), saasame::transport::g_saasame_constants.SCHEDULER_SERVICE_DISPLAY_NAME), "service display name (optional, installation only)")
        ("description", po::wvalue<std::wstring>()->default_value(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.SCHEDULER_SERVICE_DESCRIPTION), saasame::transport::g_saasame_constants.SCHEDULER_SERVICE_DESCRIPTION), "service description (optional, installation only)")
        ;
...
public:
    ...
    void worker()
    {
        ...
        int port = saasame::transport::g_saasame_constants.SCHEDULER_SERVICE_PORT;
        ...
        if (reg.open(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.CONFIG_PATH))){
            if (reg[L"ConcurrentSchedulerJobNumber"].exists() && (DWORD)reg[L"ConcurrentSchedulerJobNumber"] > 0)
                workers = (int)reg[L"ConcurrentSchedulerJobNumber"];
        ...
        _handler = std::shared_ptr<scheduler_service_handler>(new scheduler_service_handler(vm["session"].as<std::wstring>(), workers + 1));
        std::shared_ptr<TProcessor> processor(new scheduler_serviceProcessor(_handler));
...
private:
    std::shared_ptr<scheduler_service_handler> _handler;
...
#ifdef _DEBUG
                int port = saasame::transport::g_saasame_constants.SCHEDULER_SERVICE_PORT;
                std::shared_ptr<scheduler_service_handler> handler(new scheduler_service_handler(vm["session"].as<std::wstring>()));
                std::shared_ptr<TProcessor> processor(new scheduler_serviceProcessor(handler));
                std::shared_ptr<TServerTransport> serverTransport(new TServerSocket(port));
                std::shared_ptr<TTransportFactory> transportFactory(new TBufferedTransportFactory());
                std::shared_ptr<TProtocolFactory> protocolFactory(new TBinaryProtocolFactory());
                macho::windows::registry reg;
