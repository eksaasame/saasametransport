...
namespace boost { namespace application { namespace example {
   ...
   public:
      ...
      install_windows_service_(
         ...
         const setup_type<T> &service_user = setup_type<T>(L""),
         const setup_type<T> &service_password = setup_type<T>(L""),
         const setup_type<T> &service_option_string = setup_type<T>(L""))
      {
         service_name_ = service_name.get();
   ...
   protected:
      ...
      void install_service(boost::system::error_code &ec)
      {
         ...
         if ( !service_user_.empty() ) 
         {
...
#if defined(BOOST_APPLICATION_STD_WSTRING)
            if ( service_user_.find(std::basic_string<char_type>(L"\\")) ==  std::basic_string<char_type>::npos ) 
            {
               actual_service_user = std::basic_string<char_type>(L".\\") + service_user_;
            }
            else 
            {
            ...
            succeed = TRUE == ::ChangeServiceConfig(
