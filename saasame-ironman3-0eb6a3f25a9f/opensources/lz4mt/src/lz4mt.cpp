...
public:
	BlockDependentCompressor(int compressionLevel, const char* inputBuffer)
		...
		//, compressFunction(
		//	isHc ? LZ4_compressHC2_limitedOutput_continue
	...
	{
		if(isHc) {
			LZ4_resetStreamStateHC(lz4Ctx.get(), (char *)inputBuffer);
		} else {
			LZ4_resetStreamState(lz4Ctx.get(), (char *)inputBuffer);
		}
	}

	int compress(const char* source, char* dest, int inputSize, int maxOutputSize) {
        if (isHc)
            return LZ4_compressHC_limitedOutput_continue((LZ4_streamHC_t *)lz4Ctx.get(), source, dest, inputSize, maxOutputSize);
        else
            return LZ4_compress_limitedOutput_continue((LZ4_stream_t *)lz4Ctx.get(), source, dest, inputSize, maxOutputSize);
	}
