...
def humanize(hexdigest, words=4, separator='-'):
    if ('-' in hexdigest or type(hexdigest) == uuidlib.UUID):
        hexdigest = str(hexdigest).replace('-', '')
    return DEFAULT_HASHER.humanize(hexdigest, words=words, separator=separator)
