...
shared_ptr<TThreadedServer>  g_physical_packer_server;
shared_ptr<TThreadedServer>  g_proxy_server;
...
public:
    ...
    void worker()
    {
        ...
        _physical_packer_handler = shared_ptr<physical_packer_service_handler>(new physical_packer_service_handler(vm["session"].as<std::wstring>()));
        ...
        shared_ptr<TProcessor> physical_packer_processor(new physical_packer_serviceProcessor(_physical_packer_handler));
        shared_ptr<TProcessor> processor(new physical_packer_serviceProcessor(_physical_packer_handler));
        ...
        if (boost::filesystem::exists(p / "server.key") && boost::filesystem::exists(p / "server.crt"))
        {
            try
            {
                ...
                if ((!found_launcher) && session.length() && machine_id.length() && mgmt_addr.length() && (!allow_multiple || macho::windows::environment::is_winpe())){
                    shared_ptr<TServerTransport> ssl_serverTransport(new TSSLServerSocket("localhost", port, factory));
                    g_physical_packer_server = shared_ptr<TThreadedServer>(new TThreadedServer(processor, ssl_serverTransport, transportFactory, protocolFactory));
        ...
        if (!g_physical_packer_server){
            if ((!found_launcher) && session.length() && machine_id.length() && mgmt_addr.length() && (!allow_multiple || macho::windows::environment::is_winpe())){
                serverTransport = shared_ptr<TServerTransport>(new TServerSocket(port));
                LOG(LOG_LEVEL_RECORD, L"Disable listen port from \'*\'.");
            }
            g_physical_packer_server = shared_ptr<TThreadedServer>(new TThreadedServer(processor,
                serverTransport,
                transportFactory,
                protocolFactory));
        }
        if ( (!found_launcher) && session.length() && machine_id.length() && mgmt_addr.length()){
            shared_ptr<TServerTransport> proxyTransport(new TServerReverseSocket(mgmt_addr, session, machine_id, computer_name));
            g_proxy_server = shared_ptr<TThreadedServer>(new TThreadedServer(processor,
                proxyTransport,
                transportFactory,
                protocolFactory));
        }
...
private:
    ...
    shared_ptr<physical_packer_service_handler>   _physical_packer_handler;
...
#ifdef _DEBUG
                int port = saasame::transport::g_saasame_constants.PHYSICAL_PACKER_SERVICE_PORT;
                shared_ptr<physical_packer_service_handler> handler(new physical_packer_service_handler(vm["session"].as<std::wstring>()));
                shared_ptr<TProcessor> processor(new physical_packer_serviceProcessor(handler));
                shared_ptr<TServerTransport> serverTransport(new TServerSocket(port));
                shared_ptr<TTransportFactory> transportFactory(new TBufferedTransportFactory());
                shared_ptr<TProtocolFactory> protocolFactory(new TBinaryProtocolFactory());
                macho::windows::registry reg;
