...
linuxfs::io_range::map logical_volume_manager::get_system_ranges(){
    ...
    foreach(volume_group::ptr &vg, volume_groups){
        foreach(lvm_metadata& meta, metadatas){
            ...
            if (meta.pv_label_hdr_len)
                ranges_map[path].push_back(linuxfs::io_range(meta.pv_label_hdr_addr, meta.pv_label_hdr_len));
            if (meta.pv_label_len)
                ranges_map[path].push_back(linuxfs::io_range(meta.pv_label_addr, meta.pv_label_len));
            if (meta.vg_meta_len)
                ranges_map[path].push_back(linuxfs::io_range(meta.vg_meta_addr, meta.vg_meta_len));
    ...
    foreach(logical_volume::ptr lv, lvs){
        ...
        if (rw){
            ...
            if (v){
                linuxfs::io_range::vtr _ranges = v->file_system_ranges();
                if (_ranges.size()){
                    logical_volume_rw* lv_rw = dynamic_cast<logical_volume_rw*>(rw.get());
                    if (lv_rw){                
                        linuxfs::io_range::map _map = lv_rw->get_ranges_mapper(_ranges);
                        foreach(linuxfs::io_range::map::value_type &m, _map){
    ...
    foreach(logical_volume::ptr lv, unmounts){
        ...
        foreach(lv_segment &seg, lv->segments){
            ...
            foreach(lv_stripe &stripe, seg.stripes){
                ...
                if (pv && pv->rw){
                    ranges_map[pv->rw->path()].push_back(linuxfs::io_range((pv->offset + pv->pe_start + stripe_start + seg_start_sector) * SECTOR_SIZE, seg_length* SECTOR_SIZE));
                }
            }
        }
    }
        ...
        if (start_sec >= seg_start_sector && start_sec < seg_end_sector){     
            if (end_sec > seg_end_sector){
                foreach(lv_stripe &stripe, seg.stripes){
                    ...
                    if (pv && pv->rw){
                        if (result = pv->rw->sector_read(pv->offset + pv->pe_start + stripe_start + start_sec, seg_end_sector - start_sec, &(((BYTE*)buffer)[offset]), sectors_read)){
                ...
                foreach(lv_stripe &stripe, seg.stripes){
                    ...
                    uint32_t stripe_start = stripe.start_extent * _lv->extent_size;
                    if (pv && pv->rw){
                        if (result = pv->rw->sector_read(pv->offset + pv->pe_start + stripe_start + start_sec, end_sec - start_sec, &(((BYTE*)buffer)[offset]), sectors_read)){
                            number_of_sectors_read += sectors_read;
                        }
                        break;
                    }
...
linuxfs::io_range::map logical_volume_rw::get_ranges_mapper(const linuxfs::io_range::vtr &ranges){
    linuxfs::io_range::map ranges_map;
    foreach(const linuxfs::io_range &r, ranges){
...
linuxfs::io_range::map logical_volume_rw::mapper(__in uint64_t start, __in uint64_t length){
    linuxfs::io_range::map ranges_map;
