...
io_range::vtr xfs_volume::file_system_ranges(){
    ...
    foreach(linuxfs::io_range &ex, excludes){
        if (ex.start > next_start)
            ranges.push_back(linuxfs::io_range(next_start, ex.start - next_start));
        next_start = ex.start + ex.length;
    }
    if (next_start < total_size){
        ranges.push_back(linuxfs::io_range(next_start, total_size - next_start));
    }
    return ranges;
}
