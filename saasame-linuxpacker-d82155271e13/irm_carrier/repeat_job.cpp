...
#define TransportModePurggingSize 10
#define NumberOfWorkers           4

class relay_op{
public:
    relay_op(std::wstring basename, std::wstring filename, irm_imagex_local_op::ptr source_op, irm_imagex_op::vtr target_ops) :
        _filename(filename),
        _basename(basename),
        _source_op(source_op),
        _target_ops(target_ops)
    {
    }
    relay_op(std::string basename, std::string filename, irm_imagex_local_op::ptr source_op, irm_imagex_op::vtr target_ops) :
        _basename(basename),
        _source_op(source_op),
        _target_ops(target_ops)
    {
    ...
    {
        foreach(irm_imagex_op::ptr _op, _target_ops){
            std::stringstream buffer(std::ios_base::binary | std::ios_base::out | std::ios_base::in);
            irm_transport_image_blks_chg_journal::ptr p = irm_transport_image_blks_chg_journal::ptr(new irm_transport_image_blks_chg_journal());
            if (p && _op->read_metafile(_basename / (_filename + (IRM_IMAGE_JOURNAL_EXT)), *p.get())){
                return p;
            }
        }
        return NULL;
    ...
    bool replicate_transport_image_block(std::wstring transport_image_block_name, uint32_t &data, bool chip_mode)
    {
        ...
        if (result = _source_op->read_file(transport_image_block_name, ifs))
        {
            foreach(irm_imagex_op::ptr _op, _target_ops)
            {
                ...
                { 
                    if (chip_mode){
                        if (result = _op->is_file_existing(transport_image_block_name)){
                        }
                        else if (!(result = _op->write_file(transport_image_block_name, ifs))){
                            break;
                        }
                    }
                    else{
                        macho::windows::lock_able::vtr lock_objs = _op->get_lock_objects(transport_image_block_name);
                        macho::windows::auto_locks lock(lock_objs);
                        if (!(result = _op->write_file(transport_image_block_name, ifs))){
                            break;
                        }
                    }
                }
    ...
    bool replicate_journal(irm_transport_image_blks_chg_journal::ptr journal)
    {
        bool result = false;
        foreach(irm_imagex_op::ptr _op, _target_ops){
            if (!(result = _op->write_metafile(_basename / (_filename + (IRM_IMAGE_JOURNAL_EXT)), *journal.get()))){
                break;
            }
        }       
        return result;
    ...
    bool replicate_image(irm_transport_image::ptr image, bool local_only = false)
    {
        ...
        foreach(irm_imagex_op::ptr _op, _target_ops){
            if (!(result = _op->write_metafile(_basename / _filename, *img, local_only))){
                break;
            }
        }
        return result;
    ...
    bool flush_metafiles()
    {
        ...
        foreach(irm_imagex_op::ptr _op, _target_ops)
        {
            if (!(result = _op->flush_metafiles(metafiles)))
            {
                break;
            }
        }
...
struct replication_task{
    ...
    replication_task(relay_op& _op, relay_disk& _progress) : op(_op), terminated(false), progress(_progress){}
    ...
    while (true){
        {
            ...
            waitting.pop_front();
            if (IRM_IMAGE_TRANSPORT_MODE == image->mode){
                reverse_foreach(relay_block_file::ptr &b, progress.completed_blocks){
...
repeat_job::repeat_job(std::wstring id, std::string image_name, std::string parnet) :
_terminated(false),
_is_canceled(false),
...
repeat_job::vtr  repeat_job::create_jobs(std::string image_name, std::string parnet, std::set<connection>& conns)
{
    ...
    foreach(conns_map_type::value_type &c, _conns_map)
    {
        ...
        if (need_relay)
        {
            repeat_job::ptr j = repeat_job::ptr(new repeat_job(macho::guid_::create(), image_name, parnet));
            j->_conns = c.second;
...
repeat_job::ptr repeat_job::_load_config(boost::filesystem::path &config_file)
{
    ...
    try
    {
        ...
        {
            job = repeat_job::ptr(new repeat_job(id, group, parent));
            if (job)
            {
...
irm_imagex_op::vtr repeat_job::_get_target_ops()
{
    int i = 0;
...
void repeat_job::execute()
{
    ...
    irm_imagex_op::vtr        target_ops = _get_target_ops();
    if (!_is_image_uploaded(_parent)){
    ...
    else if (source_op && target_ops.size())
    {
        irm_transport_image_blks_chg_journal::ptr target_journal_db;
        relay_op op(_id, _group, source_op, target_ops);
