...
shared_ptr<TThreadedServer>  g_server;
...
bool command_line_parser(po::variables_map &vm){
    ...
    install.add_options()
        ...
        ("display", po::wvalue<std::wstring>()->default_value(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.VIRTUAL_PACKER_SERVICE_DISPLAY_NAME), saasame::transport::g_saasame_constants.VIRTUAL_PACKER_SERVICE_DISPLAY_NAME), "service display name (optional, installation only)")
        ("description", po::wvalue<std::wstring>()->default_value(macho::stringutils::convert_ansi_to_unicode(saasame::transport::g_saasame_constants.VIRTUAL_PACKER_SERVICE_DESCRIPTION), saasame::transport::g_saasame_constants.VIRTUAL_PACKER_SERVICE_DESCRIPTION), "service description (optional, installation only)")
        ;
...
public:
    ...
    void worker()
    {
        ...
        int port = saasame::transport::g_saasame_constants.VIRTUAL_PACKER_SERVICE_PORT;
        _session = vm["session"].as<std::wstring>();
        _handler = shared_ptr<virtual_packer_service_handler>(new virtual_packer_service_handler(vm["session"].as<std::wstring>()));
        shared_ptr<TProcessor> processor(new virtual_packer_serviceProcessor(_handler));
        ...
        if (boost::filesystem::exists(p / "server.key") && boost::filesystem::exists(p / "server.crt"))
        {
            try
            {
                ...
                shared_ptr<TServerTransport> ssl_serverTransport(new TSSLServerSocket(port, factory));
                g_server = shared_ptr<TThreadedServer>(new TThreadedServer(processor, ssl_serverTransport, transportFactory, protocolFactory));
        ...
        if (!g_server){
            g_server = shared_ptr<TThreadedServer>(new TThreadedServer(processor,
                serverTransport,
                transportFactory,
                protocolFactory));
        }
...
private:
    shared_ptr<virtual_packer_service_handler> _handler;
...
#ifdef _DEBUG
                int port = saasame::transport::g_saasame_constants.VIRTUAL_PACKER_SERVICE_PORT;
                shared_ptr<virtual_packer_service_handler> handler(new virtual_packer_service_handler(vm["session"].as<std::wstring>()));
