    ...
    {
        {
            ...
            reverse_foreach(replication_block::ptr& b, repository.progress.completed){
    ...
    if (!_is_interrupted)
    {
        try
        {
            ...
            else if (!(vm = portal->get_virtual_machine(virtual_machine_id)))
            {
                record((job_state::type)(_state & ~mgmt_job_state::job_state_error),
                    error_codes::SAASAME_E_CANNOT_CONNECT_TO_HOST,
                    (record_format("Cannot get virtual machine %s info.") % _create_job_detail.detail.v.virtual_machine_id));
                portal = NULL;
                _state |= mgmt_job_state::job_state_error;
            }
            else
                ...
                try{
                    ...
                    else
                    {
                        ...
                        foreach(std::string disk, _create_job_detail.detail.v.disks){
                            std::wstring wsz_disk = macho::stringutils::convert_utf8_to_unicode(disk);
                            if (!snapshot_disks.count(wsz_disk))
                            {
                                ...
                                if (!vmdk_rw)
                                {
                                    LOG(LOG_LEVEL_ERROR, L"Cannot open vmdk file (%s) for operation.", snapshot_disk->name.c_str());
                                    record((job_state::type)(_state & ~mgmt_job_state::job_state_error), saasame::transport::error_codes::SAASAME_E_IMAGE_OPEN_FAIL, record_format("Cannot open vmdk file %1%.") % boost::filesystem::path(snapshot_disk->name).filename().string());
                                    result = false;
                                    _state |= mgmt_job_state::job_state_error;
                                    break;
                                }
    ...
    if (volumes.size()){
        foreach(ntfs::volume::ptr v, volumes)
        {
            ...
            if (root)
            {
                ...
                if (_create_job_detail.file_system_filter_enable)
                {
                    ntfs::io_range::vtr vol_data = v->file_system_ranges();
                    if (vol_data.size())
                    {
                        changed_disk_extent extent;
