...
BOOL CWizardMgmt::RegisterServices()
{
    ...
    _status.SetWindowTextW(TEXT("Connecting"));
    try{
    ...
    if (Ret){
        _status.SetWindowTextW(TEXT("Connected"));
    }
    else{
        _status.SetWindowTextW(TEXT("Disconnected"));
    }
    return Ret;
}
...
void CWizardMgmt::OnClickSyslinkReset(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = 0;
    if (!_info.session.empty()){
        if (IDYES == AfxMessageBox(TEXT("Disconnect?"), MB_YESNO)){
...
BOOL CWizardMgmt::OnSetActive()
{
    ...
    pSheet->SetFinishText(_T("Connect"));
    pSheet->SetWizardButtons(PSWIZB_DISABLEDFINISH);
