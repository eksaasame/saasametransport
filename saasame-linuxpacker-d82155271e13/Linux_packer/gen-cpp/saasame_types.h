...
class loader_job_create_detail : public virtual ::apache::thrift::TBase {
 public:
  ...
  loader_job_create_detail() : replica_id(""), connection_id(), block_mode_enable(false), purge_data(true), remap(false), detect_type((disk_detect_type::type)0), worker_thread_number(0), host_name(""), export_disk_type((virtual_disk_type::type)0), export_path(""), keep_alive(true), time(""), is_continuous_data_replication(false), azure_storage_connection_string(""), thin_provisioned(true), is_paused(false) {
