...
std::vector<uint64_t> ext2_file::get_data_blocks(){
    ...
    LOG_TRACE("file_size = %llu\r\n", file_size);
    LOG_TRACE("inode.i_mode = %x\r\n", inode.i_mode);
...
uint64_t ext2_volume::extent_binarysearch(EXT4_EXTENT_HEADER *header, uint64_t lbn, bool isallocated){
    ...
    {
        LOG_ERROR("Invalid magic in Extent Header: %X", header->eh_magic);
        return 0;
    }
...
again:
    ...
    if (!newEntry)
    {
        LOG(LOG_LEVEL_ERROR, "Error reading Inode %d parent inode %d.\n", dirent->next->inode, dirent->parent->inode_num);
        return NULL;
    }
...
ext2_file::ptr ext2_volume::read_inode(uint32_t inum)
{
    ...
    {
        LOG_ERROR("Error Reading Inode %X. Invalid Inode Number", inum);
        return NULL;
    }
    LOG_TRACE("inodes_per_group =  %llu, inode_size =  %d", inodes_per_group, inode_size);
    ...
    if (!file)
    {
        LOG_ERROR("Allocation of File Failed.");
        return NULL;
    }
    src = (EXT2_INODE *)(inode_buffer.get() + inode_index);
    file->inode = *src;

    LOG_INFO("BLKNUM is %d, inode_index %d", file->inode.i_size, inode_index);
    file->inode_num = inum;
    file->file_size = (uint64_t)src->i_size | ((uint64_t)src->i_size_high << 32);
    LOG_TRACE("file->file_size is %llu\r\n", file->file_size);
    if (file->file_size == 0)
    {
        LOG_ERROR("Inode %d with file size 0", inum);
    }
    file->volume = this;
...
int ext2_volume::get_block_size() {
    return block_size;
}
...
io_range::vtr ext2_volume::file_system_ranges(){
    ...
    if (groups_desc){
        ...
        std::unique_ptr<unsigned char> umap = std::unique_ptr<unsigned char>(new unsigned char[umap_size]);
        std::unique_ptr<unsigned char> buff = std::unique_ptr<unsigned char>(new unsigned char[block_size]);
        //std::unique_ptr<unsigned char> inode_table_buff = std::unique_ptr<unsigned char>(new unsigned char[inodes_table_size]);
        if (NULL != buff.get() && NULL != umap.get() /*&& NULL != inode_table_buff.get()*/){
            ...
            for (uint64_t index = 0; index < total_groups; index++){
                ...
                uint64_t bb = get_block_bitmap(gb);
                if (bb){
            ...
            for (newBit = 0; newBit < total_blocks; newBit++){
                if (umap.get()[newBit >> 3] & (1 << (newBit & 7))){
                    ...
                    if (dirty_range){
                        ...
                        //LOG_TRACE("start = %llu,length=%llu,rw->path()=%s\r\n", start, length, rw->path().c_str());
                        ranges.push_back(io_range(start, length,start,rw));
                        dirty_range = false;
                    }
                }
            }
    ...
    if (rw->sector_read(relative_sectors + 2, 2, &sblock, number_of_sectors_read)){
        if (sblock.s_magic == EXT2_SUPER_MAGIC){         
            if (ext2fs_has_feature_compression(&sblock)){
                LOG_WARNING("File system compression is used which is not supported.");
            }
            else if (ext2fs_has_feature_bigalloc(&sblock)){
                LOG_WARNING("File system bigalloc is used which is not supported.");
            }
            else{
                ...
                memcpy(&vol->volume_uuid, &uuid, sizeof(GUID));
                //vol->volume_uuid = uuid;
                LOG_INFO("Block size %d, inp %d, inodesize %d", vol->block_size, vol->inodes_per_group, vol->inode_size);
