...
public:
    ...
    typedef boost::shared_ptr<ext2_volume> ptr;
    ...
    std::vector<uint64_t> get_all_excluded_bitmap_blocks()
    {
        FUNC_TRACER;
        std::vector<uint64_t> result;
        LOG_TRACE("total_groups = %llu", total_groups);
        for (uint64_t b = 0; b < total_groups; ++b)
        {
            LOG_TRACE("b = %llu", b);
            uint64_t bb = get_block_bitmap(get_group_desc(b));
            LOG_TRACE("bb = %llu", bb);
    ...
    std::vector<uint64_t> bitmap_blocks;
private:
    ...
    ext2_volume(universal_disk_rw::ptr &_rw, ULONGLONG _offset, ULONGLONG _size) :
        volume(_rw, _offset, _size, VOLUME_TYPE_EXT),
