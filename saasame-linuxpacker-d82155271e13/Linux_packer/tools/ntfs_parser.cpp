...
/*this function is to get the partition info, not need for linux*/
            ...
            if (NULL != v){
                ...
                if (fs){
                    ...
                    if (v->mft){
                        ...
                        LOG_WARNING("Cannot find MFT file info.");
                    }
                }
                else{
                    LOG_WARNING("Cannot find volume info.");
                }
            }
        }
    }
...
file_record::ptr volume::get_file_record(ULONGLONG fileRef, DWORD mask){
    ...
    while (ahc->Type != (DWORD)-1 && (dataPtr + ahc->TotalSize) <= file_record_size){
        if (ATTR_MASK(ahc->Type) & mask)	// Skip unwanted attributes
        {
            ...
            if (is_encrypted() || is_compressed()){
                LOG_RECORD("Compressed and Encrypted file not supported yet!");
                return false;
            }
        }
...
bool file_record::parse_attr(PNTFS_ATTRIBUTE ahc){
    ...
    if (attrIndex < ATTR_NUMS){
        ...
        if (attr){
            ...
            LOG_ERROR("Attribute Parse error: 0x%04X", ahc->Type);
        }
    }
    else{
        LOG_ERROR("Invalid Attribute Type: 0x%04X", ahc->Type);
    }
    return false;
}
        ...
        if (pick_data(&dataRun, &length, &LCNOffset)){
            ...
            if (dr){
                ...
                if (dr->last_vcn <= (ahc->Attr.NonResident.LastVCN - ahc->Attr.NonResident.StartVCN)){
                    ...
                    LOG_ERROR("DataRun decode error: VCN exceeds bound");
                    return false;
                }
            }
            else
...
bool file_record::pick_data(const BYTE **dataRun, LONGLONG *length, LONGLONG *LCNOffset){
    ...
    if (lengthBytes > 8 || offsetBytes > 8){
        LOG_ERROR("DataRun decode error 1: 0x%02X", size);
        return false;
    }
    *length = 0;
    memcpy(length, *dataRun, lengthBytes);
    if (*length < 0){
        LOG_ERROR("DataRun length error: %I64d", *length);
        return false;
    }
...
bool attributes::read_virtual_clusters(ULONGLONG vcn, DWORD clusters,
    ...
    *actural = 0;
    ...
    if (vcn + clusters > (ahc->Attr.NonResident.LastVCN - ahc->Attr.NonResident.StartVCN + 1)){
        LOG_ERROR("Cluster exceeds DataRun bounds");
        return false;
    }
...
bool attributes::read_clusters(void *buf, DWORD clusters, LONGLONG lcn){
    if (lcn == ULLONG_MAX){	// sparse data
        LOG_RECORD("Sparse Data, Fill the buffer with 0");
    ...
    if (rw->read(addr + offset, clusters*cluster_size, buf, len) &&
        ...
        LOG_ERROR("Cannot read cluster with LCN %I64d", lcn);
    }
    return false;
}
...
file_record::ptr volume::find_sub_entry(const std::string file_name, const file_record::ptr dir){
    file_record::ptr _dir = dir;
    if (NULL == _dir)
        _dir = root();
    if (_dir->attrs.count(ATTR_INDEX(ATTR_TYPE_INDEX_ROOT))){
        for(attributes::ptr attr : _dir->attrs[ATTR_INDEX(ATTR_TYPE_INDEX_ROOT)]){
                ...
                while (ieTotal <= IndexRoot->TotalEntrySize){
                    if (ie->Flags & INDEX_ENTRY_FLAG_SUBNODE){
                        ...
                        if (visit_index_block(_dir, *(ULONGLONG*)((BYTE*)ie + ie->Size - 8), file_name, fileRef)){
                            return get_file_record(fileRef);
                        }
                    }
                    if (ie->StreamSize){
                        ...
                        if ((file_name.length() == filename->NameLength) &&
                            (0 == strncasecmp((char*)filename->Name, file_name.c_str(), filename->NameLength))){ //maybe problem
                            return get_file_record(ie->FileReference & 0x0000FFFFFFFFFFFFUL);
                        }
                    }
...
bool volume::visit_index_block(const file_record::ptr dir, const ULONGLONG vcn, const std::string& file_name, ULONGLONG& fileRef){
        ...
        if (NULL != ibBuf.get()){
            ...
            if (attr->read_data(vcn *index_block_size, ibBuf.get(), index_block_size, &len) &&
                ...
                if (!patch_us((WORD*)ibBuf.get(), sectors, usn, usarray)){
                    LOG_ERROR("Index Block parse error: Update Sequence Number");
                    return false;
                }
                INDEX_ENTRY *ie;
...
#endif
                        if ((file_name.length() == filename->NameLength) &&
                            (0 == strncasecmp((char*)filename->Name, file_name.c_str(), filename->NameLength))){ //maybe issue here
...
file_extent::vtr file_record::extents(){
    ...
    if (attrs.count(ATTR_INDEX(ATTR_TYPE_DATA))){
        ...
        if (attr->ahc->NonResident){
            for(data_run::ptr dr : attr->data_runs){
