...
bool get_volume_retrieval_points(boost::filesystem::path _p){
    bool result = false;
    std::string p = _p.string();
    if (p[p.length() - 1] == '\\')
        p.erase(p.length() - 1);
    macho::windows::auto_handle handle = CreateFileA(p.c_str(),
        GENERIC_READ,
    ...
    if (handle.is_invalid()){
        LOG(LOG_LEVEL_ERROR, L"CreateFile (%s) failed. error: %d", _p.wstring().c_str(), GetLastError());
        return result;
    }
    else{
        ...
        if (NULL == pVcnPairs.get()){
            ...
            while (!DeviceIoControl(handle, FSCTL_GET_RETRIEVAL_POINTERS, &starting, sizeof(STARTING_VCN_INPUT_BUFFER), pVcnPairs.get(), ulOutPutSize, &BytesReturned, NULL)){
                if (ERROR_BUFFER_OVERFLOW == GetLastError()){
                    ...
                    LOG(LOG_LEVEL_ERROR, L"DeviceIoControl(FSCTL_GET_RETRIEVAL_POINTERS) failed. Error: %d", GetLastError());
                    return result;
                }
            }
            ...
            LOG(LOG_LEVEL_RECORD, L"Excluded the data replication for the file (%s).", _p.wstring().c_str());
            result = true;
        }
    }
    return result;
...
class mirror_disk{
public:
    mirror_disk() : _is_interrupted(false), _block_size(MAX_BLOCK_SIZE), _mini_block_size(MINI_BLOCK_SIZE), _backup_size(0){}
...
bool mirror_disk::replicate_changed_areas(universal_disk_rw::ptr &rw, macho::windows::storage::disk& d, uint64_t& start_offset, uint64_t offset, uint64_t& backup_size, changed_area::vtr &changeds, universal_disk_rw::ptr& output){
    ...
    foreach(changed_area &c, changeds){
        if (result = replicate(rw, c.start_offset, c.length, output, offset)){
            if (output && _backup_size){
                start_offset = c.start_offset + c.length + offset;
            ...
            foreach(macho::windows::storage::partition::ptr p, parts){
                ...
                for (changed_area::vtr::iterator c = changed.begin(); c != changed.end();){
                    if (c->start_offset <= start_offset){
                        if (!(is_replicated = replicate(disk_rw, c->start_offset, c->length, output, 0)))
                            break;
                        c = changed.erase(c);
                    }
                    else
                ...
                if (start_offset < end_of_partiton && d.partition_style() == macho::windows::storage::ST_PARTITION_STYLE::ST_PST_GPT && (macho::guid_(p->gpt_type()) == macho::guid_(PARTITION_SYSTEM_GUID))){
                    ...
                    if (d.partition_style() == macho::windows::storage::ST_PARTITION_STYLE::ST_PST_MBR &&
                        ...
                        is_replicated = replicate(disk_rw, start_offset, 1024 * 1024, output);
                    }
                    else{
                        changed_area::vtr  changeds = get_changed_areas(disk_rw, d, (*p.get()), start_offset);
                        if (!(is_replicated = replicate_changed_areas(disk_rw, d, start_offset, 0, backup_size, changeds, output)))
